...
class ObjectDetector(object):
    def __init__(self, input_img, feature_extractor, model, nms, image_pyramid=[140, 200, 256], window_step=16, window_height=128, window_width=64, prob_threshold=0.9):
    ...
    def __call__(self):
        ...
        for box in pboxes:
            cv2.imwrite(os.path.join(output_path, 'crop_%s.jpg' % time.time()), d_img[box[1]:box[3], box[0]:box[2], :])
        ...
        cv2.imwrite(os.path.join(output_path, 'done.jpg'), d_img)
